...
import finalGame.inventoryStuff.DroppedItem;
...
{
	public double percentBroken = 0; // how broken the block is
	public double hardness = 120; // how hard the block is to break
	...
	public Block block; // this will have the characteristics of the block like the color the type the interactions you can have with it
	...
	{ // we don't want to put that all into the block class so now we can make a bunch of these classes and have it be more consise
		...
		{
			try{
				breaking[0] = toCompatibleImage(ImageIO.read(new File("res/images/break_0.png")));
				breaking[1] = toCompatibleImage(ImageIO.read(new File("res/images/break_1.png")));
				breaking[2] = toCompatibleImage(ImageIO.read(new File("res/images/break_2.png")));
			}catch(IOException e){}
			loaded = true;
		}
	}
	...
	public void breakBlock(double input)
	{
		if(breakable)
		{
			percentBroken += 1/hardness * input;
			if(percentBroken >1)
			{
